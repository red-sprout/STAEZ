window.onload = function () {
    ...
    communityCategory({ refCategoryNo: refCategoryMap.others, categoryLevel: 1 }, (res) => {
        const navArr = [{ categoryNo: 0, categoryName: "커뮤니티 메인" }, ...res];
        setCategory(res, 'notice-category')
        setChecked("category");
    });
...
function setChecked(name) {
    ...
    const categoryBtn = document.querySelectorAll(`#notice-${name}>.btn-staez`);
    const categoryRadio = document.querySelectorAll(`#notice-${name} input[name='categoryNo']`);
