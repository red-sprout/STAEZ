$(function () {
    ...
    document.querySelector("#admin-answer-inquire").addEventListener("click", answerInquireEvent);
    document.querySelector("#admin-delete-inquire").addEventListener("click", deleteInquireEvent);
})
...
// Inquire 목록 그리기
function drawInquireList(inquireList) {
    const tbody = document.getElementById("admin-contents");
    for (let board of inquireList) {
...
function answerInquireEvent() {
    const boardList = getCheckboxChecked();
    if (boardList.length !== 1) {
        alert("수정시 한 게시글만 선택하기 바랍니다.")
        return;
    }
    location.href = `inquireInsertForm.ad?boardNo=${boardList[0]}`
}